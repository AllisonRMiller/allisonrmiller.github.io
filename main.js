...
function initBlog(){
    ...
    var unpack = blog[0][1].map((x) => {return `<p>` +x + `</p>`});
    document.getElementById("blog").innerHTML= unpack;
    document.getElementById("blogtitle").innerText = title;
}
initBlog(blog);

function blogClick(e){
    console.log("click fired",e.target.id);
    var page = e.target.id;
    console.log(page);
    console.log(blog[page][0]);
    var title = blog[page][0];
    var unpack = blog[page][1].map((x) => {return `<p>` +x + `</p>`}).join("");
    document.getElementById("blog").innerHTML= unpack;
    document.getElementById("blogtitle").innerText = title;
}
